...
%    %% TODO: change this! we need to pull height to state level and give a threshold (or e.g. fee threshold)
...
handle_micro_signing_reply({{error, {runtime, Reason}}, _}, State) ->
    ...
    epoch_mining:error("Failed to mine block, runtime error; "
                       "retrying with different nonce (was ~p). "
                       "Error: ~p", [Candidate#candidate.nonce, Reason]),
    %% TODO: implement retry-micro-signing to potentially re-generate micro block, do we need that?
    start_micro_signing(State).
...
%% TODO: we need to be more relaxed here
%% TODO: check accumulated fees - to see if we should mine
%% TODO: consider coinbase award a function of fees. Miners are incentivise to include microblocks then
%% TODO: XXX fix prev!!! it may be oldTopHash
%%
%% here I removed checking the prev-hash
%%
    ...
    case aec_chain:has_block(Hash) of
        ...
        false ->
            ...
            case {aec_headers:validate(Header, LeaderKey), aec_blocks:validate(Block, LeaderKey)} of
                {ok, ok} ->
                    %%
                    %% TODO: XXX for key block make chain accept the block and handle micro-forks
...
setup_loop(State, LeaderKey, block_received) ->
    ...
    start_mining(State1);
setup_loop(State, no_key, micro_block_created) ->
...
handle_post_header(Header, #state{consensus = #consensus{leader_key = LeaderKey}} = State) ->
    ...
    case aec_chain:has_header(Hash) of
        true ->
            ...
            {ok, State};
        false ->
            case aec_headers:validate(Header, LeaderKey) of
                ok ->
