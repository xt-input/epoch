...
new_node_joins_network(Cfg) ->
    ...
    setup_nodes([?OLD_NODE1, ?OLD_NODE2, ?NEW_NODE1]),

    %% Starts a chain with two nodes
    start_node(old_node1),
    start_node(old_node2),
    wait_for_value({height, 0}, [old_node1, old_node2], NodeStartupTime),

    %% Mines for 20 blocks and calculate the average mining time
    StartTime = os:timestamp(),
    wait_for_value({height, Length}, [old_node1, old_node2], Length * ?MINING_TIMEOUT),
    EndTime = os:timestamp(),
    %% Average mining time per block plus 50% extra
    MiningTime = round(timer:now_diff(EndTime, StartTime) * 1.5)
                 div (1000 * Length),

    Top1 = request(old_node1, [v2, 'top'], #{}),
    ct:log("Node 1 top: ~p~n", [Top1]),
    Height1 = request(old_node1, [v2, 'block-by-height'], #{height => Length}),
    ct:log("Node 1 at height ~p: ~p~n", [Length, Height1]),
    Height2 = request(old_node2, [v2, 'block-by-height'], #{height => Length}),
    ...
    wait_for_value({height, 0}, [new_node1], NodeStartupTime),
    ct:log("Node 3 ready to go"),

    %% Waits enough for node 3 to sync but not for it to build a new chain
    wait_for_value({height, Length}, [new_node1], MiningTime * 3),
    ct:log("Node 3 on same height"),
    Height3 = request(new_node1, [v2, 'block-by-height'], #{height => Length}),
...
docker_keeps_data(Cfg) ->
    ...
    setup_nodes([?STANDALONE_NODE]),

    start_node(standalone_node),
    wait_for_value({height, 0}, [standalone_node], NodeStartupTime),

    %% Mines for 20 blocks and calculate the average mining time
    StartTime = os:timestamp(),
    wait_for_value({height, Length}, [standalone_node], Length * ?MINING_TIMEOUT),
    EndTime = os:timestamp(),
    %% Average mining time per block plus 50% extra
    MiningTime = round(timer:now_diff(EndTime, StartTime) * 1.5)
                 div (1000 * Length),
    ...
    stop_node(standalone_node, infinity), %% Is this triggering PT-155851463 ?
    start_node(standalone_node),
    wait_for_value({height, 0}, [standalone_node], NodeStartupTime),
    ...
    wait_for_value({height, Length + 10}, [standalone_node], MiningTime * 10),

    %% Get all blocks before stopping
    C = [get_block(standalone_node, H) || H <- lists:seq(1, Length + 10)],

    stop_node(standalone_node, infinity),
    start_node(standalone_node),
    wait_for_value({height, 0}, [standalone_node], NodeStartupTime),
...
stop_and_continue_sync(Cfg) ->
    ...
    setup_nodes([?OLD_NODE1, ?OLD_NODE2]),

    start_node(old_node1),
    wait_for_value({height, 0}, [old_node1], NodeStartupTime),

    StartTime = os:timestamp(),
    wait_for_value({height, Length}, [old_node1], Length * ?MINING_TIMEOUT),
    EndTime = os:timestamp(),
    %% Average mining time per block plus 50% extra
    MiningTime = round(timer:now_diff(EndTime, StartTime) * 1.5)
                 div (1000 * Length),

    B1 = request(old_node1, [v2, 'block-by-height'], #{height => Length}),
    ct:log("Node 1 at height ~p: ~p~n", [Length, B1]),

    %% Start fetching the chain
    start_node(old_node2),
    wait_for_value({height, 0}, [old_node2], NodeStartupTime),
    ct:log("Node 2 ready to go"),

    %% we are fetching blocks stop node1 now
    kill_node(old_node1),
    Top2 = request(old_node2, [v2, 'top'], #{}),
    ...
    case Height >= Length of
         ...
         false ->
            start_node(old_node1),
            wait_for_value({height, Length}, [old_node2], (Length - Height) * MiningTime),
            B2 = request(old_node2, [v2, 'block-by-height'], #{height => Length}),
...
%% is already shared.
net_split_recovery(_Cfg) ->
    Length = 10,

    setup_nodes([?NET1_NODE1, ?NET1_NODE2, ?NET2_NODE1, ?NET2_NODE2]),
    start_node(net1_node1),
    start_node(net1_node2),
    start_node(net2_node1),
    start_node(net2_node2),

    %% Starts with a net split

    wait_for_value({height, Length}, [net1_node1, net1_node2, net2_node1, net2_node2],
                    Length * ?MINING_TIMEOUT),

    A1 = request(net1_node1, [v2, 'block-by-height'], #{height => Length}),
    A2 = request(net1_node2, [v2, 'block-by-height'], #{height => Length}),
    A3 = request(net2_node1, [v2, 'block-by-height'], #{height => Length}),
    A4 = request(net2_node2, [v2, 'block-by-height'], #{height => Length}),
    ...
    %% Join all the nodes
    connect_node(net1_node1, net2),
    connect_node(net1_node2, net2),
    connect_node(net2_node1, net1),
    connect_node(net2_node2, net1),

    wait_for_value({height, Length * 3}, [net1_node1, net1_node2, net2_node1, net2_node2],
                    Length * 2 * ?MINING_TIMEOUT),

    B1 = request(net1_node1, [v2, 'block-by-height'], #{height => Length * 3}),
    B2 = request(net1_node2, [v2, 'block-by-height'], #{height => Length * 3}),
    B3 = request(net2_node1, [v2, 'block-by-height'], #{height => Length * 3}),
    B4 = request(net2_node2, [v2, 'block-by-height'], #{height => Length * 3}),
    ...
    disconnect_node(net1_node1, net2),
    disconnect_node(net1_node2, net2),
    disconnect_node(net2_node1, net1),
    disconnect_node(net2_node2, net1),

    wait_for_value({height, Length * 5}, [net1_node1, net1_node2, net2_node1, net2_node2],
                    Length * 2 * ?MINING_TIMEOUT),

    C1 = request(net1_node1, [v2, 'block-by-height'], #{height => Length * 5}),
    C2 = request(net1_node2, [v2, 'block-by-height'], #{height => Length * 5}),
    C3 = request(net2_node1, [v2, 'block-by-height'], #{height => Length * 5}),
    C4 = request(net2_node2, [v2, 'block-by-height'], #{height => Length * 5}),
    ...
    D1 = request(net1_node1, [v2, 'block-by-height'], #{height => Length * 7}),
    D2 = request(net1_node2, [v2, 'block-by-height'], #{height => Length * 7}),
    D3 = request(net2_node1, [v2, 'block-by-height'], #{height => Length * 7}),
    D4 = request(net2_node2, [v2, 'block-by-height'], #{height => Length * 7}),
...
sync_speed(_Cfg) ->
    ...
    setup_nodes(InitialNodeSpecs ++ NextNodeSpecs),
    [start_node(N) || N <- InitialNodes],
    wait_for_value({height, Height}, InitialNodes, MiningTimeout * Height),

    InitialBlocks = [
        request(N, [v2, 'block-by-height'], #{height => Height})
        || N <- InitialNodes
    ],

    start_node(node_name(5)),
    wait_for_value({height, 0}, [node_name(5)], ?STARTUP_TIMEOUT),
    wait_for_value({height, Height}, [node_name(5)], ?SYNC_TIMEOUT * Height),
    N5Block = request(node_name(5), [v2, 'block-by-height'], #{height => Height}),

    start_node(node_name(6)),
    wait_for_value({height, 0}, [node_name(6)], ?STARTUP_TIMEOUT),
    wait_for_value({height, Height}, [node_name(6)], ?SYNC_TIMEOUT * Height),
    N6Block = request(node_name(6), [v2, 'block-by-height'], #{height => Height}),
...
get_block(NodeName, Height) ->
    Query = #{height => Height},
    case http_get(NodeName, ext_http, [v2, 'block-by-height'], Query) of
        {ok, 200, Block} -> Block;
        {ok, 404, _} -> undefined
    end.
